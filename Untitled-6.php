...
// ✅ Brevo Mail Function
function sendMail($to, $subject, $message, $from="info@itcoaching.com") {
    $apiKey = "YOUR_BREVO_API_KEY"; // 🔑 Replace with your Brevo API key
    $url = "https://api.brevo.com/v3/smtp/email";

    $data = [
        "sender" => ["email" => $from, "name" => "IT Coaching Institute"],
        "to" => [["email" => $to]],
        "subject" => $subject,
        "textContent" => $message
    ];
...
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['submit_form'])){
    ...
    // Send mail to owner
    $owner_email = "info@itcoaching.com"; // Change this to your email
    $owner_subject = "New Enrollment: $name";
    $owner_body = "Name: $name\nEmail: $email\nPhone: $phone\nCourse: $course\nMessage: $message";

    $owner_mail_sent = sendMail($owner_email, $owner_subject, $owner_body);

    // Send confirmation mail to student
    $student_subject = "Enrollment Confirmation - IT Coaching Institute";
    $student_body = "Hi $name,\n\nThank you for enrolling in our course: $course.\nWe will contact you soon with more details.\n\nBest Regards,\nIT Coaching Institute";

    $student_mail_sent = sendMail($email, $student_subject, $student_body);

    if($owner_mail_sent && $student_mail_sent){
        $success = true;
    } else {
        echo "<p style='color:red;'>Mail sending failed. Please contact admin directly.</p>";
    }
}
...
<main class="container">
  ...
  <section id="contact">
    ...
    <?php if($success) echo "<div class='success-msg'>Your submission has been received & emails sent!</div>"; ?>
    <form method="post">
